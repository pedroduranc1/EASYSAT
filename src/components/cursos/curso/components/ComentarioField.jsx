...
export const ComentarioField = ({ id }) => {
  ...
  const {data:ComentData,isLoading,error} = useQuery(`Coments ${id}`,()=> ComentsCtrl.getComent(id));
  ...
  return (
    ...
    >
      {!screenWidth < 750 && ComentData?.length > 0 && (
      ...
      <div
        ref={ContentRef}
        className="w-fit relative flex gap-x-3 h-full transition-all"
      ...
      >
        ...
        <AnimatePresence>
          {ComentData?.length > 0 ? (
            <>
              {ComentData.map((coment, index) => (
                <ComentarioCard key={index} Autor={coment.refId} Comentario={coment} />
              ))}
            </>
          ): (
            <div className="w-full flex  justify-center items-center mb-10">
              <h2 className="text-2xl font-bold text-white">No existe comentarios disponibles</h2>
            </div>
          )}
        </AnimatePresence>
      </div>
