...
import { VideoCard } from "../cursos/CardVideo";
...
const BlogCarousel = ({ array, isPopular }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className="w-[95%]  overflow-auto sm:overflow-hidden relative flex h-full transition-all"
      >
        <div 
        ref={ContentRef}
        className="transition-all flex w-fit"
        style={{ transform: `translateX(${scrollPosition}px)` }}
        >
          {/* CARD BLOG */}
          <AnimatePresence>
            {array.map((blog, index) => (
              <VideoCard key={index} blog={blog} />
            ))}
          </AnimatePresence>
        </div>
      </div>
